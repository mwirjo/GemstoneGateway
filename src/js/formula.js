// formula.js - 13 Mineral Categories → Show NAME → Guess FORMULA!
...
function setupFormulaGame() {
  ...
  if (checkBtn) checkBtn.addEventListener("click", checkFormula);
...
function checkFormula() {
  ...
  if (!answer) {
    showFeedback("Enter the chemical formula!", "error");
    return;
  }
  
  const mineral = currentChallenge[currentIndex];
  const isCorrect = mineral.formula?.toUpperCase() === answer;
  ...
  showFeedback(
    isCorrect ? 
      `✅ CORRECT! ${mineral.name}: ${mineral.formula} (${mineral.category})` :
      `❌ Wrong! ${mineral.name}: ${mineral.formula} (${mineral.category})`,
  ...
  if (feedback) {
    feedback.innerHTML = `
      <div class="feedback-result success">
        <h3>🎉 5-CATEGORY FORMULA COMPLETE!</h3>
